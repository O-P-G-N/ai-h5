<template>
	<view class="startcreat">
			...
			<view class="mymodelmain">
				<view class="touming">
					...
					<view class="jiaoyimubiao">
						<text>{{$t("ac.prc2")}}</text>
						<text class="jiaoyimubiao_text" v-if="pageData.dictSort==1">{{$t("ac.share")}}</text>
		...
		<view class="createbtn">
			<ai-button :btnHeight="'50px'" :bg="'#333'" :disabled="forbidden" :loading="loading" class="chuangjian"
				@click="createContract">{{$t("ac.prc14")}}</ai-button>
		...
		methods: {
			...
			getDayRevenue(val) {
				uni.request({
					...
					method: "GET",
					success: (res) => {
						this.revenueList = res.data.list.sort(function(a, b) {
							return function(a,b){
								if (a.days < b.days) {
										return -1;
			...
			createContract() {
				...
				// if(that.from.payNum>Math.floor((Number(that.balanceMax)/Number(that.pageData.remark)))){
